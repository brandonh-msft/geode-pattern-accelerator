...
resource "null_resource" "apimservice" {
  provisioner "local-exec" {
    command = "az apim create --name ${local.service_name} -g ${var.resource_group_name} -l ${var.location} --sku-name Consumption --sku-capacity 1 --publisher-email publisher@example.com --publisher-name Publisher"
  }
}
